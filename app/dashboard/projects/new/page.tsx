...
import { DEVNET_PACKAGE_ID } from "@/lib/constants";
const categories = [
  "DeFi",
  "NFTs",
  "Gaming",
...
export default function NewProjectPage() {
  ...
  function addMilestone() {
    ...
    setMilestones((arr) => [
      ...arr,
      { title: mTitle.trim(), percent: Number(mPercent), endDate: mDate },
    ]);
    setMTitle("");
    setMPercent("");
    setMDate("");
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      const packageId = DEVNET_PACKAGE_ID;
      if (!packageId) {
      ...
      const projectLink =
        (formData.get("projectLink") as string | null)?.trim() ?? "";
      const fundingGoalRaw =
        (formData.get("fundingGoal") as string | null)?.trim() ?? "";
      const fundingDeadlineRaw =
        (formData.get("fundingDeadline") as string | null)?.trim() ?? "";
      ...
      if (!milestones.length) {
        throw new Error(
          "Add at least one milestone before creating a project.",
        );
      }
      ...
      if (
        milestoneDeadlines.some((deadline) => deadline <= fundingDeadlineMs)
      ) {
        throw new Error(
          "Milestone deadlines must be after the funding deadline.",
        );
      }
      ...
      tx.moveCall({
        ...
        arguments: [
          ...
          tx.pure.vector(
            "u64",
            milestoneDeadlines.map((deadline) => BigInt(deadline).toString()),
          ),
              ...
              >
                ...
                <div className="md:col-span-2 flex items-start gap-3 rounded-md border p-4">
                  <Checkbox
                    id="closeOnGoal"
                    checked={closeOnGoal}
                    onCheckedChange={(value: boolean) => setCloseOnGoal(value === true)}
                    disabled={submitting}
                  />
                  <div className="space-y-1">
                    <Label htmlFor="closeOnGoal">
                      Close funding when goal is reached
                    </Label>
                    <p className="text-xs text-muted-foreground">
                      If enabled, new pledges are disabled once the funding goal
                      is met.
                    </p>
                  </div>
                </div>
